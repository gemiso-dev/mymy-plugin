"""브라우저 핸드오프 인증 (loopback).

로컬 콜백 서버를 127.0.0.1에 띄우고 브라우저로 MYMY 웹의 /mcp-auth를 연다.
사용자가 "연결 허용"을 누르면 웹이 발급받은 mym_ 토큰을 콜백으로 **top-level 폼 POST
네비게이션**으로 전달한다. (공개 origin 페이지 → loopback 으로의 fetch는 브라우저 Private
Network Access 정책상 비보안 컨텍스트에서 차단되므로 fetch가 아닌 폼 네비게이션을 쓴다.)
...
def _find_chrome() -> str | None:
    ...
    candidates = [
        ...
        os.environ.get("MYMY_CHROME_PATH"),
...
def browser_handoff(web_url: str, timeout: float = 180.0) -> dict:
    """브라우저 핸드오프로 MYMY mym_ 토큰을 받아 반환한다.

    반환: {"access": str}  (access = mym_ API 토큰)
    실패 시 TimeoutError.
    """
    import secrets
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _html(self, code: int, message: str):
            body = (
                "<!doctype html><meta charset=utf-8><title>MYMY</title>"
        ...
        def do_POST(self):
            ...
            if not host_hdr.startswith("127.0.0.1"):
            ...
            if data.get("state") != state or not data.get("access"):
    ...
    url = (
        f"{web_origin}/mcp-auth?cb={urllib.parse.quote(cb)}"
        f"&state={state}&host={urllib.parse.quote(host)}"
    )
